...
export const NavMenu = () => {
  ...
  const setCurrent = () => {
    for (let i = 0; i < 10; i++) {
      !!!i18next.language &&
        setTimeout(() => {
          setIsUkrainian(ukrainianLang.includes(i18next.language));
        }, 500);
    }
  };

  const [isUkrainian, setIsUkrainian] = useState(
        ...
        <Container>
          ...
          <Collapse
            ...
            <NavbarBrand tag={Link} to='/'>
              <Translation>{(t) => t('navmenu.hompage')}</Translation>
            </NavbarBrand>
            <ul className='navbar-nav flex-grow'>
              <NavItem>
                <NavLink tag={Link} className='text-dark' to='/documents'>
                  <Translation>{(t) => t('navmenu.documents')}</Translation>
                </NavLink>
              </NavItem>
              <NavItem>
                <NavLink tag={Link} className='text-dark' to='/resources'>
                  <Translation>{(t) => t('navmenu.help')}</Translation>
                </NavLink>
              </NavItem>
              <NavItem>
                <a
...
91 110 140 141 110 71 269 91 392 49z'
              ...
              {isUkrainian ? (
                <>
                  ...
                  <NavItem>
                    <span
                      className='text-dark nav-link current-language'
            ...
            <a className='chose-bouquet-btn-nav' href='#chose'>
              <Translation>{(t) => t('button.chooseBouquet')}</Translation>
            </a>
          </Collapse>
        </Container>
      </Navbar>
